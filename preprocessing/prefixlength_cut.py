...
if __name__ =='__main__':
    ...
    for x in range(2,11):
        length = x
        df = pd.read_csv('data/dataset/bpic/bpic2017/BPIC17_dur.csv')
        print('Prefix :', length)
        df = morethan(df,length)
        dir_path = 'data/dataset/bpic/bpic2017/prefix'+str(length)
        try:
            os.makedirs(dir_path)
        except:
            pass

        df.to_csv(dir_path+'/bpic_prep.csv',index=False)
