...
if __name__=='__main__':
    for k in range(2,11):
        ...
        dirname = 'data/dataset/bpic/bpic2017/prefix'+str(prefix)
        df = pd.read_csv(dirname+'/bpic_prep.csv')
        
        df = df.rename(columns = {'(case) CreditScore':'CreditScore', 
                       '(case) FirstWithdrawalAmount':'FirstWithdrawalAmount', '(case) MonthlyCost':'MonthlyCost',
                        '(case) NumberOfTerms':'NumberOfTerms','(case) OfferedAmount':'OfferedAmount', '(case) Selected':'Selected', '(case) Accepted':'Accepted' })
        
        case_att_con = ['CreditScore','FirstWithdrawalAmount','MonthlyCost', 'NumberOfTerms','OfferedAmount']
        #case_att_cat = ['Selected']
        event_att_cat = ['Activity', 'Resource']
        #event_att_con = ['case_cost']

        
        savedir = 'data/dataset/bpic/bpic2017/prefix'+str(prefix)
        ...
        dfs = [timediscretize(df,prefix),y_value(df,y_column),event_att_cat_onehot(df,event_att_cat,prefix), case_att_con_onehot(df,case_att_con)]
